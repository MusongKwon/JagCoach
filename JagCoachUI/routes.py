...
from firebase.firebase_utils import retrieve_evals, upload_video_to_firebase, save_transcript, save_eval, retrieve_eval, \
    get_next_filename, delete_upload
import time
...
def evaluate():
    ...
    try:
        global processed_audio_path
        global filler_ratio
        future_elements = process_executor.submit(get_elements, processed_audio_path)
        ...
        print("getting elements dictionary complete")
        print(student_results)

        print("Starting Evaluation")
        evaluation_text = evaluate_speech(student_results)
        print("Evaluation complete bossman")
        save_eval(user_email, evaluation_text, next_filename)
